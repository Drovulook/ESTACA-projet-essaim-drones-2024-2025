function setupZones(app)
    ...
    for i = 1:length(authZoneCoord)
        authZones{end+1} = Zone(authZoneType{i}, 'A', authZoneCoord{i}, authZoneDims{i}, authZoneTilt{i}, app.env);
        app.env.addZone(authZones{i}); % Ajouter la zone autorisée à l'environnement
    end
    ...
    for i = 1:length(proZoneCoord)
        proZones{end+1} = Zone(proZoneType{i}, 'P', proZoneCoord{i}, proZoneDims{i}, proZoneTilt{i}, app.env);
        app.env.addZone(proZones{i}); % Ajouter la zone prohibée à l'environnement
    end
    ...
    for i = 1:length(manZoneCoord)
        manZones{end+1} = Zone(manZoneType{i}, 'M', manZoneCoord{i}, manZoneDims{i}, manZoneTilt{i}, app.env);
        app.env.addZone(manZones{i}); % Ajouter la zone de manœuvre à l'environnement
    end
end